...
impl Simulator {
    ...
    pub fn add_node<T : 'static + PrintableElement>(&mut self, element : T) -> usize {
        ...
        self.node_index_to_node.insert(self.elements.len(), node_index);
        self.add_element(element)
    }


    pub fn add_edge<T : 'static + PrintableElement>(&mut self, edge : T, element1 : usize, element2 : usize) -> usize {
    ...
    pub fn add_one_direction_edge<T : 'static + PrintableElement>(&mut self, edge : T, element1 : usize, element2 : usize) -> usize {
        ...
        for src in 0..self.elements.len() {
            for (rpc, dst) in &self.rpc_buffer[src] {
                if (*dst).is_some() {
                    ...
                    if self.elements[src].whoami().0 == "Link" || self.elements[src].whoami().0 == "TrafficGenerator" {
