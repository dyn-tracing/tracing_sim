...
impl Filter {
    ...
    pub fn execute(&mut self, x: &Rpc) -> Option<Rpc> {
        ...
        if my_node == String::from("0") {
            ...
            let vertices = vec![ String::from("m"), String::from("n"),   ];
            ...
            if mapping.len() > 0 {
                ...
                let count_value = count_obj_ptr.execute().to_string();
                
                fs::write("result.txt", count_value).expect("Unable to write file");
        ...
        let count_state_ptr = self.filter_state.get_mut("count").unwrap();
        let count_obj_ptr = count_state_ptr.udf_count.as_mut().unwrap();
        count_obj_ptr.execute().to_string();
