...
impl Filter {
    ...
    pub fn store_headers(&mut self, x: Rpc) {
        ...
        if x.headers.contains_key("properties_path") {
            if self.envoy_shared_data.contains_key(&prop_str) {
                ...
                self.envoy_shared_data
                    .get_mut(&prop_str)
                    .unwrap()
                    .push_str(&x.headers["properties_path"]);
                self.envoy_shared_data
                    .get_mut(&prop_str)
                    .unwrap()
                    .push_str(";");
                self.envoy_shared_data
                    .get_mut(&prop_str)
                    .unwrap()
                    .push_str(self.whoami.as_ref().unwrap());
            } else {
                // add yourself
        ...
        for key in &self.collected_properties {
            ...
            if x.headers.contains_key(&prop_key) {
                if self.envoy_shared_data.contains_key(&prop_str) {
                    // concatenate with comma if not duplicate
                    // make lists of properties
                    let properties_unified = self.envoy_shared_data[&prop_str].clone();
                    let mut properties: Vec<String> = properties_unified
                        .split(",")
                        .map(|s| s.to_string())
                        .collect();
                    let cur_properties_unified = x.headers[&prop_key].clone();
                    let mut cur_properties: Vec<String> = cur_properties_unified
                        .split(",")
                        .map(|s| s.to_string())
                        .collect();
                    ...
                    self.envoy_shared_data
                        .insert(prop_str.clone(), property_string_to_store.clone());
                } else {
                    self.envoy_shared_data
                        .insert(prop_str.clone(), x.headers[&prop_key].clone());
                }
            }
        }
    }
    ...
    pub fn merge_headers(
        ...
        if new_rpc_headers["direction"] == "response" {
            ...
            if self.envoy_shared_data.contains_key(&prop_str) {
                new_rpc_headers.insert(
                    "properties_path".to_string(),
                    self.envoy_shared_data[&prop_str].clone(),
                );
            } else {
                new_rpc_headers.insert(
                    "properties_path".to_string(),
    ...
    pub fn on_incoming_requests(&mut self, mut x: Rpc) -> Vec<Rpc> {
        let mut prop_str;
        prop_str = format!(
            "{whoami}.{property}=={value}",
            whoami = &self.whoami.as_ref().unwrap(),
            property = "service_name",
            value = self.filter_state["node.metadata.WORKLOAD_NAME"]
                .string_data
                .as_ref()
                .unwrap()
                .to_string()
        ...
        if x.headers.contains_key("properties_service_name") {
            if !x.headers["properties_service_name"].contains(&prop_str) {
                // don't add our properties if they have already been added
                x.headers
                    .get_mut(&"properties_service_name".to_string())
                    .unwrap()
                    .push_str(",");
                x.headers
                    .get_mut(&"properties_service_name".to_string())
            ...
            x.headers
                .insert("properties_service_name".to_string(), prop_str);
        }
    ...
    pub fn on_outgoing_responses(&mut self, mut x: Rpc) -> Vec<Rpc> {
        ...
        if x.headers.contains_key("properties_height") {
            if !x.headers["properties_height"].contains(&height_str) {
                // don't add a udf property twice
                x.headers
                    .get_mut(&"properties_height".to_string())
                    .unwrap()
                    .push_str(",");
                x.headers
                    .get_mut(&"properties_height".to_string())
            ...
            x.headers
                .insert("properties_height".to_string(), height_str);
        }

        let mut trace_graph = self.create_trace_graph(x.clone());
        let mapping =
            find_mapping_shamir_centralized(&trace_graph, self.target_graph.as_ref().unwrap());
        if !mapping.is_none() {
            let m = mapping.unwrap();
            let mut value = "0".to_string(); // dummy value
                                             // TODO: do return stuff
            let node_ptr =
                utils::get_node_with_id(&self.target_graph.as_ref().unwrap(), "a".to_string());
            if node_ptr.is_none() {
            ...
            let mut ret_height = &trace_graph
                .node_weight(trace_node_index.unwrap())
                .unwrap()
                .1["height"];
            ...
            result_rpc.headers.insert("dest".to_string(), dest);
            result_rpc
                .headers
