...
pub fn get_value_for_storage(                                                   
    ...
    let node_ptr = graph_utils::get_node_with_id(target_graph, "a".to_string());
        ...
        if trace_node_index == None || !&fd.trace_graph.node_weight(trace_node_index.unwrap()).unwrap().1.contains_key("height") {
            // we have not yet collected the return property or have a mapping error
            return None;
        }
        let ret_height = &fd.trace_graph.node_weight(trace_node_index.unwrap()).unwrap().1[ "height" ];
