...
impl Filter {
    ...
    pub fn execute(&mut self, x: &Rpc) -> Vec<Rpc> {
        ...
        .join(".");
        data_to_append = self.filter_state[&data_key]
            .string_data
            .as_ref()
            .unwrap()
            .to_string();
            ...
            let mapping = isomorphic_subgraph_matching(
                ...
                |x, y| {
                    for property in y.1.keys() {
                        if property != &"node.metadata.WORKLOAD_NAME".to_string()
                            && &(x.1[property]) != &(y.1[property])
                        {
                            return false;
                        }
                    }
